...
const Skills = ({background, lightMode, reduceMotion}) => {
  ...
  const [mouseReset, setMouseReset] = useState(false)
  ...
  useEffect(() => {
    ...
    const mouseCap = (e) => {
      if ((!isVisible || reduceMotion) && !mouseReset) {
        doMouseReset();
      } else {
        const newDummyBox = ["","","",""];
  ...
  }, [centerX, centerY, isVisible, mouseCoord, reduceMotion, midX, midY, offsetX, offsetY, shineOp, dummyBoxArray, mouseReset])
